...
structure_fire_per_1k = function(data_year){
  ...
  incidents = fread("NFIRS_2009_2016.txt")
  colnames(incidents) = toupper(colnames(incidents))
  inc = as.POSIXct(incidents[,INC_DATE], format = "%m/%d/%Y")
  incidents = incidents[year(inc) == data_year,]
  
  building_fire = incidents[grep('111', INC_TYPE)]
  
  
  # need to cast INC_DATE as int type so that incidenta and address can be joined
  address$INC_DATE = as.integer(address$INC_DATE)
  
  setkey(incidents, STATE, FDID, INC_DATE, INC_NO, EXP_NO)
  setkey(address, STATE, FDID, INC_DATE, INC_NO, EXP_NO)
  
  building_fire_address = address[incidents[,grep('111', INC_TYPE)]]
  ...
  # needed for map
  saveRDS(tract_census_selected_normal_population, file = paste0(data_year, '_tract_population_fire.rds'))
  write.csv(tract_census_selected_normal_population, file = paste0(data_year, '_tract_building_fire_per_1k.csv'))
}
...
tracts_covered_all_years = c(tracts_covered_all_years,
                           readRDS(paste0(year, '_tracts_covered.rds')))
}
...
tracts_census_national = tracts_census_national[, .(STATE10, GEOID10, POP10)]
...
#tracts_census_national = tracts_census_national[!duplicated(tracts_census_national$GEOID10),]
...
# determines which entries in the census data are also in the fire tract data
covered_tracts = tracts_census_national[GEOID10 %in% as.numeric(tracts_covered_all_years_unique),]
...
write.csv(states_coverage, file = 'states_coverage.csv')

...
structure_fire_per_1k = function(data_year){
  ...
  # the following line is how originals read in data
  # load(paste0(project_data, 'address/', data_year, '_formated_addresses.Rdata')) 
  
  # I read in data like so
  address = fread("./2013_formated_addresses.csv", nrows = 10)
  ...
  # the following line is how originals read in data
  # load(paste0(project_data, 'address/', data_year, '_geocoded_addresses.Rdata')) 
  
  # I read in data like so
  geocoded_address = fread("./2013_geocoded_addresses.csv", nrows = 10)
  ...
  basic_incident = fread("./basicincident.txt", nrows = 10, sep = "^")
  ...
  fire_incident = fread("./NFIRS_FireIncidents.csv", nrows = 10)
  ...
  # the following line of code was the originals
  ...
  # the following line of code comes from originals
  # building_fire_address = address[basic_incident[grep('111', INC_TYPE)]]
  
  # I used the following
  building_fire_address = address[basicincident[,grep('111', INC_TYPE)]]
  
  # Orders the order of the building addresses based on which order they appear
  # (I'm assuming row_seq means its row location in the data set)
...
# I read it in like
tracts_census_national = fread("./us2010trf.txt", sep = ",", header = FALSE, nrows = 10)
...
states_county_fips = fread("./national_county.txt", sep = ",", header = FALSE, nrows = 10)

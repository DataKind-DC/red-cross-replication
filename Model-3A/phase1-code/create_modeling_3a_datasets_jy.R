...
for(year in years) {
  ...
  ### Read in the data
  data_formatted = fread(paste0(downloads_path, year, "_formated_addresses.csv"), data.table = FALSE)
  data_geocoded = fread(paste0(downloads_path, year, "_geocoded_addresses.csv"), data.table = FALSE)
  ...
  if(year %in% c("2009", "2010", "2011")) {
    nfirs_incident_data = read.dbf(paste0(downloads_path,  "/basicincident.dbf"))
  }
  
  else if(year %in% c("2012", "2013")) {
    nfirs_incident_data = fread(paste0(downloads_path,  "/basicincident.txt"), sep = "^",
                              data.table = FALSE)
  }
  ...
  ### Save cleaned data
  save(nfirs_outcomes_geo_clean, file = paste0(save_path, "nfirs_outcomes_geo_clean_", year, ".Rdata"))
...
load("nfirs_outcomes_geo_clean_2012.Rdata")
...
load("nfirs_outcomes_geo_clean_2013.Rdata")
...
save(nfirs_outcomes_geo_clean_multi_year, file = paste0(save_path, "nfirs_outcomes_geo_clean_multi_year.Rdata"))
...
rm(list=ls())
load("nfirs_outcomes_geo_clean_multi_year.Rdata")
...
source("/Users/judyyang/GitHub/red-cross-2/Model-3a/phase1-code/general_purpose/unbalanced_downsample.r")
...
#Not sure acs is same as acs_tract_data.csv
acs_tract_data <- fread('http://enigma-public.s3.amazonaws.com/projects/smoke-alarm-risk/data/acs.csv')
...
write.csv(modeling_death_dataset_imp, "modeling_death_dataset_imp.csv",  row.names = FALSE)
...
write.csv(modeling_injury_dataset_imp, "modeling_injury_dataset_imp.csv",     row.names = FALSE)
